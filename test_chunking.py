...
RE_CONTENU = (
    r"(?:parl\w*(?:\s+(?:de|d['’]|du|des))?|"
    r"trait\w*(?:\s+(?:de|d['’]|du|des))?|"
    r"\bsur\b(?:\s+(?:la|le|l['’]|les|un|une|des))?|"
    r"à\s+propos\s+(?:de|d['’]|du|des)|"
    r"[ée]voqu\w*(?:\s+(?:la|le|l['’]|les|un|une|des))?|"
    r"mentionn\w*(?:\s+(?:la|le|l['’]|les|un|une|des))?|"
    r"concern\w*(?:\s+(?:la|le|l['’]|les|un|une|des))?|"
    r"port\w*\s+sur(?:\s+(?:la|le|l['’]|les|un|une|des))?|"
    r"impliqu\w*(?:\s+(?:la|le|l['’]|les|un|une|des))?|"
    r"liés?\s+à(?:\s+la)?)"
)

RE_NEGATION = r"\b(?:(?:mais\s+)?(?:pas|non|sans)\b(?:\s+(?:de|d['’]|du|des|la|le|l['’]|les|un|une))?)"

RE_STOP = (
    r"(?:dans\s+(?:la\s+)?rubrique|"
    r"de\s+(?:la\s+)?rubrique|"
    r"pour\s+rubrique|"
    r"dont\s+la\s+rubrique|"
    r"\brubrique\b|"
    r"écrits?\s+(?:en|entre|après|avant)|"
    r"publiés?\s+(?:en|entre|après|avant|au\s+mois)|"
    r"parus?\s+(?:en|entre|après|avant)|"
    r"datés?\s+(?:de|du|entre|après|avant|à\s+partir\s+de)|"
    r"datent\s+(?:de|du|entre|après|avant)|"
    r"de\s+l['’]année|"
    r"\b20\d{2}\b|"
    r"au\s+mois\s+de|"
    r"\bmois\s+de|"
    ...
    r"contenant\s+une\s+image|"
    r"sans\s+image|"
    r"provenant\s+de\s+(?:la\s+)?rubrique|"
    r"date\s+d['’]après|"
...
combined_regex = "(" + "|".join([f"(?P<grp_{i}>{pattern})" for i, (pattern, _) in enumerate(triggers)]) + ")"
compiled_regex = re.compile(combined_regex, re.IGNORECASE)

for s in sentences:
    matches = list(compiled_regex.finditer(s))
    chunks = []
    last_end = 0
    current_type = "NONE"
    ...
    for m in matches:
        start, end = m.span()
        if start > last_end:
            content = s[last_end:start].strip()
            if content:
                chunks.append((current_type, content))
                
        grp_name = m.lastgroup
        idx = int(grp_name.split("_")[1])
        current_type = triggers[idx][1]
        last_end = end

    if last_end < len(s):
        content = s[last_end:].strip()
        if content:
            chunks.append((current_type, content))
            
    print(f"\n--- {s}")
    for t, c in chunks:
        if t in ["TITRE", "CONTENU", "EXCLUDE"]:
            print(f"[{t}] {c}")
