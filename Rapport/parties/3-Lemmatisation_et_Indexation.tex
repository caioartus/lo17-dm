...
L'ensemble de la pipeline de nettoyage du corpus (anti-dictionnaire et lemmatization) est réalisée par la classe \texttt{DataCleaner}. Cette classe se sert de la classe \texttt{AntiDict} pour determiner les stop words et d'un \texttt{Stemmer} (\texttt{SpacyStemmer} par default), pour lemmatiser le text dans le texte et le titre (les autres champs sont laissés tel quel). \texttt{DataCleaner} génère un tableau de substitution avec des pairs \texttt{token}/\texttt{lemma} mais ou \texttt{lemma} peut être vide si c'est un stop word.
\begin{figure}[h]
    \centering
