...
TRIGGERS = [
    (r"(?i)(?:dont\s+le\s+)?titre\s+contient(?:\s+les?\s+mots?)?", "TITRE"),
    (r"(?i)(?:dont\s+le\s+)?titre\s+[ée]voque", "TITRE"),
    (r"(?i)contenant\s+les?\s+mots?(?:\s+dans\s+le\s+titre)?", "TITRE"),
    (r"(?i)possédant\s+le\s+mot", "TITRE"),
    
    (r"(?i)parl\w*(?:\s+(?:de|d['’]|du|des))?", "CONTENU"),
    (r"(?i)trait\w*(?:\s+(?:de|d['’]|du|des))?", "CONTENU"),
    (r"(?i)\bsur\b(?:\s+(?:les?|la|l['’]|un|une|des))?", "CONTENU"),
    (r"(?i)\bà\s+propos\s+(?:de|d['’]|du|des)", "CONTENU"),
    (r"(?i)[ée]voqu\w*(?:\s+(?:les?|la|l['’]|un|une|des))?", "CONTENU"),
    (r"(?i)mentionn\w*(?:\s+(?:les?|la|l['’]|un|une|des))?", "CONTENU"),
    (r"(?i)concern\w*(?:\s+(?:les?|la|l['’]|un|une|des))?", "CONTENU"),
    (r"(?i)port\w*\s+sur(?:\s+(?:les?|la|l['’]|un|une|des))?", "CONTENU"),
    (r"(?i)impliqu\w*(?:\s+(?:les?|la|l['’]|un|une|des))?", "CONTENU"),
    (r"(?i)liés?\s+à(?:\s+la)?", "CONTENU"),
    
    (r"(?i)(?:mais\s+)?(?:pas\s+de|non\s+pas|sans)(?:\s+(?:de|d['’]|du|des|les?|la|l['’]|un|une))?", "EXCLUDE"),
    
    (r"(?i)(?:dans\s+(?:la\s+)?|de\s+(?:la\s+)?|dont\s+la\s+|pour\s+)?rubrique(?:s)?", "OTHER"),
    (r"(?i)(?:écrits?|publiés?|parus?|datés?|datent)\s+(?:en|entre|après|avant|du|de|à\s+partir\s+de)", "OTHER"),
    (r"(?i)de\s+l['’]année", "OTHER"),
    (r"(?i)\b20\d{2}\b", "OTHER"),
    (r"(?i)au\s+mois\s+de|\bmois\s+de", "OTHER"),
    (r"(?i)(?:qui\s+ont|possédant|avec|contenant)\s+(?:des\s+)?images?", "OTHER"),
    (r"(?i)date\s+d['’]après", "OTHER"),
    (r"(?i)le\s+\d{1,2}\s+(?:janvier|février|mars|avril|mai|juin|juillet|août|septembre|octobre|novembre|décembre)", "OTHER"),
    (r"(?i)\d{2}/\d{2}/\d{4}", "OTHER"),
]
...
for s in sentences:
    print(f"\n--- {s}")
    for match in re.finditer(combined_regex, s):
        grp_name = match.lastgroup
        if grp_name:
            idx = int(grp_name.split("_")[1])
            print(f"[{TRIGGERS[idx][1]}] {match.group()}")
